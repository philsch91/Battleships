...
public class Field {
    ...
    private boolean isAreaFree(int x, int y, int length, Direction dir) {
        for (int i = 0; i < length; i++) {
            /**
             * Hier, nimmt es die Koordinaten und prüft ob es innerhalb vom Spielfeld liegt.
             * Wenn nicht, returned er false und isAreaFree
             * liefert in der setShip Methode false zurück
             * (was dann passiert, steht in der setShip Methode)
             */
            if (x < 0 || x > 9 || y < 0 || y > 9) {
            ...
            /**
             * Wenn beide if-Bedienungen true zurück liefern,
             * erhöhen wir entweder die x oder y Koordinate abhängig von der Richtung.
             * Wenn das Schiff nach oben zeigt, müssen wir y-- machen, um den nächsten 40
             * Pixelblock (== 1 ShipPart) zu überprüfen, ob da ein Schiff gesetzt werden darf.
             * Das machen wir alles so lang, wie die Länge von dem Schiff, das wir setzen wollen. (For-Schleife)
             */
            switch (dir) {
                case UP:
                    y--;
    ...
    /**
     * Es zählt wie viele Schiffe es in der Länge schon gibt, in der wir gerade anlegen wollen.
     * Nimmt hier aber noch keine Rücksicht darauf, ob es schon 4 in der Länge 2 z.B schon gibt.
     * Das passiert erst in der setShip Methode bzw isFleetComplete.
     * @param length
     * @return
     */
    ...
    /**
     * Liefert true zurück, wenn die folgenden Bedienungen in der Klammer,
     * nach dem return, erfüllt sind.
     * Sprich, wenn shipCount für alle(!!) Schiffslängen die richtige Anzahl gezählt hat (z.B für length 2 ==4(Schiff)), dann true.
     * @return
     */
	...
	public boolean setShip(int x, int y, int length, Direction dire, int diffvectorx, int diffvectory) {
        ...
        /**
         * Zuerst überprüfen wir, ob die Anzahl der Schiffe,
         * mit der Länge die wir gerade setzen wollen, nicht eh schon
...
Schiff zutreffen und checkIfDestroyed (Ship-Klasse) true liefert, returned es das zerstörte Schiff, ansonsten null.*/
    ...
    /**
     * Verwendung: reset Methode in der Main.
     * Wenn reset aufgerufen wird, wird removeAll aktiviert, bedeutet, dass wir
